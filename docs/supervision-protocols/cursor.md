...
7. The turn-end guard (`bin/fm-turnend-guard.sh`, via the cursor shim) remains the final backstop.
   ...
   The `loop_limit: null` registration leaves continuation budget to the shim, which keeps one session-scoped chain record in `state/.cursor-turnend-chain` (keys `session`, `total`, `fail`, `wake`, `reason`, reset on session mismatch): consecutive loud arm-failure follow-ups (`FM_CURSOR_TURNEND_BLOCK_BUDGET`, default 3) and consecutive repeats of the same canonical wake reason (`FM_CURSOR_WAKE_CHAIN_BUDGET`, default 5, with one final diagnostic follow-up at the ceiling).
   Dynamic stale and check diagnostics stay attached to their stable wake identity, so changing diagnostic details do not evade re-fire bounds; a different wake reason is progress and does not add to `total`, while repeated reasons and failures advance the unified count.
