...
7. The turn-end guard (`bin/fm-turnend-guard.sh`, via the cursor shim) remains the final backstop.
   ...
   A different wake reason is progress and does not add to `total`, so ordinary back-to-back supervision events never exhaust the chain; each kind resets the other, repeated reasons and failures advance the unified count, an allowed stop or a non-continuation stop (`loop_count` 0, a captain-driven turn) clears the record, and the ceiling diagnostic clears it too so the next chain starts from a normal follow-up.
