...
7. The turn-end guard (`bin/fm-turnend-guard.sh`, via the cursor shim) remains the final backstop.
   ...
   A different wake reason is progress and restarts the wake count, so ordinary back-to-back supervision events never exhaust the chain; each kind resets the other, an allowed stop or a non-continuation stop (`loop_count` 0, a captain-driven turn) clears the record, the ceiling diagnostic clears it too so the next chain starts from a normal follow-up, and an exhausted failure budget resets rather than silencing the banner permanently.
   One unified hard bound sits over both: a chain whose `loop_count` reaches `FM_CURSOR_TURNEND_BLOCK_BUDGET + FM_CURSOR_WAKE_CHAIN_BUDGET` ends, so an arm flapping between actionable and failed closes cannot run forever.
   If the record cannot be persisted (read-only or full `state/`), the payload's `loop_count` carries the per-branch bound as well.
